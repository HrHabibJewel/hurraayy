...
const Photos = (props) => {
  ...
  const expPhotoId = getExperiencePhotoId();
  ...
  const getPhotos = () => {
    $axios.get("experience_photo/experience/" + experienceId).then(
      function (res) {
        //console.log("experience imgs", res);
        res &&
          res.data &&
      ...
      (err) => {
        console.error("err experience imgs");
      }
    );
  };
  ...
  function postUpload(coverPhoto = false, file) {
    ...
    $axios
      .post("experience_photo/experience/" + experienceId, formData)
      .then(function (resp) {
        if (resp && resp.status == 200) {
          setPhotoId(resp.data["id"]);
          //console.log("===photoId==", resp.data['id'])
         // localStorage.setItem("experiencePhoto", JSON.stringify(resp));
          dispatch(experienceForm("photo", experienceId, resp.data["id"]));
        }
      });
  }
  ...
  function putUpload(coverPhoto = false, file) {
    ...
    $axios
      .put(
        "experience_phot/" + expPhotoId + "/experience/" + experienceId,
        formData
      )
      .then(function (resp) {
        if (resp && resp.status == 201) {
          dispatch(experienceForm("photo", experienceId, expPhotoId));
        }
      });
  }
  const imgsUpload = (img) => {
    //console.log("image", img);
    //console.log("expPhotoId", expPhotoId);
    setImgs([...imgs, ...img]);
