...
const WillProvide = (props) => {
  ...
  const getProvideList = () => {
    if(groupProvide.length > 0 && selectedGroup) {
      $axios.get(`experience_provide_group/${selectedGroup}/event-list-by-provide`).then(function (resp) {
  ...
  useEffect(() => {
    ...
    getExperienceFormEditData().then((resp) => {
      if(resp && resp['providingAnythingForGuest']) {
       ...
       if(resp && resp['experienceProvideGroups']){
         let _data = [];
         let kArr = [];
        for(let key in resp['experienceProvideGroups']) {
          if(resp['experienceProvideGroups'][key]['experienceProvideVO']) {
            let groupId = resp['experienceProvideGroups'][key]['experienceProvideVO']['id'];
            if(!_data[groupId]) {
              kArr[groupId] = 0
              _data[groupId] = [];
              _data[groupId][kArr[groupId]] = resp['experienceProvideGroups'][key]['id']
            } else {
              kArr[groupId]++;
              _data[groupId][kArr[groupId]] = resp['experienceProvideGroups'][key]['id']
            }
  ...
  function getGroupProvides() {
    $axios.get("/experience_provide").then((resp) => {
      if(resp && resp.status == 200) {
  ...
  const handleSubmit = (e) => {
    ...
    $axios.put("experience/"+experienceId+"/provide/", JSON.stringify(params)).then(function (resp) {
      if (resp && resp.status == 200) {
        dispatch(
          experienceForm("will-provide", experienceId)
        );
        closeModal();
    ...
    <Spin spinning={loading} tip="Loading...">
      <Form onSubmit={handleSubmit}>
        ...
        <div className="exp_con">
          {/* <h2>Add details about what you’ll provide</h2> */}
          <h2>Items included in the experience package</h2>
          <p>
          Your experience may include food, drinks, equipment, transportation etc. Mention what is included in the package. 
          </p>
        </div>
        <div className="exp_con">
          {/* <Collapse
