...
export default ({ data }) => {
  ...
  const getSchedule = (dateRange = [], popUp = 0) => {
    ...
    $openAxios
      .get("adventure-open/adventure/" + id + "/schedule", { params: params })
      .then((resp) => {
        if (resp.status == 200) {
          if(popUp == 0){
        ...
        <div>
          {
            schedule && schedule.length > 0 && schedule.map((item, key) => {
              let month = moment(item['startDate']).format('MMMM')
              let day = moment(item['startDate']).format('DD')
            ...
            {scheduleForPopUp &&
              ...
              scheduleForPopUp.map((item, key) => {
                ...
                return (
                  ...
                  //       <strong className="d-block">{month}</strong>
                  //       {/* <small className="d-block">Starts 9am</small> */}
                  //     </p>
                  //   </div>
                  //   <div className="col-6 my-auto">
