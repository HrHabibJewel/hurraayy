...
const WellBe = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    validateFields((err, values) => {
      ...
      params["locationIds"] = "";
      $axios.put("adventure/" + adventureId + "/about-where-will-be/", JSON.stringify(params)).then(function (resp) {
        if (resp && resp.status == 200) {
          dispatch(
            adventureHost("will-be", adventureId)
          );
  ...
  return (
    <Spin spinning={loading} tip="Loading...">
      <Form onSubmit={handleSubmit}>
        ...
        <div className="exp_con">
          <h6>You may be visiting several places in the adventure. Mention each place with name and short description.</h6>
          <Form.Item>
