...
const IdeaTheme = (props) => {
  ...
  const getTheme = () => {
    $axios.get("adventure_themes").then(function (resp) {
  ...
  const getEventList = () => {
    if (mainTheme1.length > 0 && selectedMainTheme) {
      $axios.get("adventure_theme_groups/theme-groups/" + selectedMainTheme).then(function (resp) {
        if (resp && resp.status == 200) {
          let _groupEvent = [];
          ...
          if (_groups && _groups.length > 0) {
            for (let key in _groups) {
              ...
              let _eventList = [];
              let _events = _groups[key]['adventureThemeEventVOs'];
  ...
  const openModal = () => {
    getAdventureHostEditData().then((resp) => {
      if (resp && resp['adventureThemeEventVOS'] && resp['adventureThemeEventVOS'].length > 0) {
        let _themeId = 0;
        let _eventId = '';
        for (let key in resp['adventureThemeEventVOS']) {
          if (resp['adventureThemeEventVOS'][key] && resp['adventureThemeEventVOS'][key]['adventureThemeVO']) {
            _themeId = resp['adventureThemeEventVOS'][key]['adventureThemeVO']['id']
          }
          _eventId = resp['adventureThemeEventVOS'][key]['id'];
  ...
  const onContinue = () => {
    ...
    const next = props.continue;
    $axios.put("adventure/" + adventureId + "/main-theme/", JSON.stringify(params)).then(function (resp) {
      if (resp && resp.status == 200) {
        dispatch(
          adventureHost("theme", adventureId)
        );
        closeModal();
  ...
  return (
    <Spin spinning={loading} tip="Loading...">
      <div>
        <Form.Item>
          <Button htmlType="submit" style={{ float: "right" }} onClick={() => {
      ...
      <Form onSubmit={handleSubmit}>
        <div className="exp_con">
          <h2>Select the category of the Adventure</h2>
          <p>
            Pick a category that closely matches your adventure.
          </p>
        </div>
        <div className="exp_con">
          <Form.Item>
                ...
                mainTheme1.map((item, key) => {
                  return (
                    <div className="col-6 p-2" key={key} style={{ cursor: "pointer" }} onClick={() => themeActiveClass(item)}>
                      <div className="exp-theme-card" style={(selectedMainTheme == item['id']) ? { border: '1px solid red' } : {}}>
