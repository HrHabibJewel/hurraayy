...
const MeetingLocation = (props) => {
  ...
  useEffect(() => {
    ...
    getAdventureHostEditData().then((resp) => {
      if (resp && resp['adventureLocaton']) {
        let _data = resp['adventureLocaton'];
  ...
  const handleSubmit = (e) => {
    ...
    validateFields((err, values) => {
      if (!err) {
        ...
        $axios.put("adventure/" + adventureId + "/meeting_location/", JSON.stringify(params)).then(function (resp) {
          if (resp && resp.status == 200) {
            dispatch(
              adventureHost("meeting-location", adventureId)
            );
    ...
    <Spin spinning={loading} tip="Loading...">
      ...
      <Form onSubmit={handleSubmit}>
        <div className="exp_con">
          ...
          <p>This is normally the starting point of your adventure. Mention the exact address from where the adventure will start.</p>
        </div>
