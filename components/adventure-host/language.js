...
const Language = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    validateFields((err, values) => {
      ...
      $axios.put("adventure/" + adventureId + "/language/", JSON.stringify(params)).then(function (resp) {
        if (resp && resp.status == 200) {
          dispatch(
            adventureHost("language", adventureId)
          );
    ...
    <Spin spinning={loading} tip="Loading...">
      <Form onSubmit={handleSubmit}>
        ...
        <div className="exp_con">
          ...
          {/* <Form.Item>
            {getFieldDecorator("languageAdventure", {
              initialValue: lanAdventure,
              rules: [
                {
                  required: false,
                },
