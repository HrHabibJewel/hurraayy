...
export default () => {
    ...
    const getAdventureList = () => {
        $axios.get('experience/going-where').then((resp) => {
    ...
    const columns = [
        {
            title: 'Trips',
            render: record => <p className="normal-text">{record.experience.title}</p>,
        },
        {
            title: 'Dates',
            render: record => <p className="normal-text">{record.blockDate}</p>,
        },
        {
            title: 'Total Going',
