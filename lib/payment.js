...
export default (function () {
  ...
  const paymentModalOpen = function () {
    document
      .querySelector("#ssl-payment-modal .payment-modal-container")
      .classList.add("payment-active");
  ...
  const paymentModalHide = function () {
    document
      .querySelector("#ssl-payment-modal .payment-modal-container")
      .classList.remove("payment-active");
  ...
  const modalEvent = function () {
    document.querySelector("#ssl-payment-modal").addEventListener(
      "click",
      function (event) {
    ...
    window.addEventListener("message", function (event) {
      if (
        event.origin === "https://sandbox.sslcommerz.com" ||
        event.origin === "https://epay.sslcommerz.com"
      ) {
        ...
        if (data.type == "resize") {
          document.querySelector(
            "#ssl-payment-modal .payment-modal-body iframe"
          ).style.height = parseFloat(data.height) + 5 + "px";
        }
      }
    });
  };
